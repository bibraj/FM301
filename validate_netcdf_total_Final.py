...
def generate_pdf(results, output_file, section_summaries,results_data, nc_file):
    ...
    story.append(Paragraph("Measurements, Instrumentation and Traceability Section, WIGOS Division, WMO ", styles['Normal']))
    doc.build(story)
...
if __name__ == "__main__":
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print("Usage: python validate_netcdf.py <metadata.json> <file.nc> <report.pdf> [f for full sweep]")
        sys.exit(1)

    json_file, nc_file, pdf_file = sys.argv[1], sys.argv[2], sys.argv[3]
    Sopt = sys.argv[4].lower() if len(sys.argv) == 5 else "o"  # default = "f"
